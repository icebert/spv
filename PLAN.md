...
**Security pass for the public Internet (2026-09-17).** Threat model: hostile `.h5ad` files (from `#url=`, the URL box or disk) and hostile share links; the supply chain; hosting headers. Findings and fixes: `#url=` accepted any scheme (`javascript:`, `data:`, `blob:` reached `fetch`), so `src/util/safeUrl.ts` now allows http(s) only, checked in `App.open()` for every URL source, and `resolveUrl` no longer passes `blob:` through. CSV/TSV exports wrote file-derived text verbatim; `src/util/csv.ts` prefixes formula-leading cells with an apostrophe and flattens tabs and newlines in TSV. `readImage` allocated before checking dimensions; it now refuses images above `MAX_IMAGE_SAMPLES` (512 M) up front. `el()` sets `style` through `node.style.cssText` (CSSOM, which CSP does not govern), so the policy dropped `'unsafe-inline'` for styles entirely (`style-src 'self'`; the dev server still adds `'unsafe-inline'` for Vite's injected `<style>` elements). Already sound and left alone: every string from a file reaches the DOM as a text node (the only `innerHTML` is the static icon paths); file colours must match a hex regex in `readCategoryColors`; URL-state keys are parsed one by one with regex or enumeration checks, so no prototype pollution; download names are reduced to `[A-Za-z0-9_-]`; cross-origin fetches carry no credentials. Supply chain: `.npmrc` sets `ignore-scripts=true` (no installed package has an install script today, so a future one fails loudly), CI runs `npm audit signatures`, and both workflows use commit-pinned actions (`checkout` v7.0.1, `setup-node` v7.0.0, `upload-pages-artifact` v5.0.0, `deploy-pages` v5.0.1, `upload-artifact` v6.0.0) with `persist-credentials: false`. `SECURITY.md` states the model and the reporting path; `docs/hosting.md` lists the headers to send when self-hosting and asks Pages owners to enforce HTTPS. Trusted Types were considered and not enabled: `new Worker(url)` is a Trusted Types sink and would need a policy in code for a Chromium-only gain.

**Tablet pass (2026-09-17).** Checked in Chromium with iPad emulation (810×1080, 1080×810, 834×1194, touch, coarse pointer) and CDP touch events: one-finger orbit, two-finger pinch, tap-to-pin and touch lasso already worked (OrbitControls handles touch; the canvas has `touch-action: none`). Changes: an `@media (pointer: coarse)` block raises buttons, tabs, list and legend rows, inputs and the tooltip/toast close buttons to at least 36 px; `pressable()` treats a 450 ms touch hold as a Shift-click and swallows the trusted click and context menu that follow, so legend entries and sections can be soloed without a keyboard; the selection bar shows Lasso and Box buttons (Shift-drag still forces a box); the Appearance panel gains a Fullscreen button when `document.fullscreenEnabled` is true (iPhone Safari has no element fullscreen, so the button simply does not appear there); the pinned tooltip is placed above the touch point on coarse pointers; the sheet layout now applies up to 900 px so portrait iPads (768 to 834 px) get the full-width view instead of a 490 px column; `viewport-fit=cover` plus `env(safe-area-inset-*)` padding on `#app`; `user-select: none` and `-webkit-touch-callout: none` on the view and the controls. Three E2E tests run under the iPad device profile: layout and target sizes, gestures through `Input.dispatchTouchEvent`, long-press solo and Box-mode touch selection.

**Security verification pass (2026-09-17).** A second look at the public-Internet posture after the first security commit. Re-checked and found sound: the only `innerHTML` is the static icon table; no `eval`, `new Function`, `window.open`, `postMessage` listeners on `window`, or navigation from data; `fetch` never sends credentials cross-origin; `npm audit` (runtime and dev) and `npm audit signatures` are clean; no secrets or `.env` files in the tree. Headless WebKit (Playwright's Safari engine, `Version/26.6`) loads the demo under the shipped policy with zero `securitypolicyviolation` events: like Chromium it gives the same-origin worker its own (empty) policy, so WebAssembly compiles without a `wasm-unsafe-eval` allowance on the page. Tightened: the deploy workflow now grants `pages: write` and `id-token: write` to the deploy job only (the build job, which runs npm packages and Playwright, keeps a read-only token); share-link numbers that fail to parse fall back to the defaults instead of injecting `NaN` into the state, `decodeIndexList` caps at 100 000 entries in total, and alignment entries with unparsable numbers are skipped; `data/datasets.json` is validated entry by entry (`manifestEntries`), so a typo hides one dataset with a warning instead of breaking the picker. An opt-in Playwright project (`npm run e2e:webkit`) runs the CSP and URL-refusal tests in WebKit. `docs/hosting.md` now asks Pages owners to protect `main` (required CI check, restricted push), since every push there deploys.
