...
- **Content Security Policy.** `index.html` carries a `<meta http-equiv="Content-Security-Policy">` (GitHub Pages cannot send headers): scripts, workers, styles and fonts only from the site itself, no inline scripts or styles (element styles go through the CSSOM), no plugins or embedded objects. `connect-src` stays open because `#url=` may point at any CORS-enabled host; only http(s) targets are accepted. The dev server allows inline styles because Vite injects CSS as `<style>` elements; builds and `vite preview` use the strict policy, and the end-to-end suite fails on any violation. Nothing is loaded from third parties: the typeface, the WASM engine and the compression plugins ship with the site, and the page sends no `Referer` header. The threat model and the reporting process are in [SECURITY.md](../SECURITY.md).
- **GitHub Pages.** Turn on **Enforce HTTPS** in the Pages settings. Pages sends no `X-Frame-Options`, so the site can be framed; the viewer keeps no session state, so a framed copy can only show what the framing page could load itself.
