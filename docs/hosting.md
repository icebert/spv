...
Pull requests and pushes to other branches run the same gates in `.github/workflows/ci.yml` without deploying: `npm audit` on the runtime dependencies, `npm audit signatures` (registry signatures and provenance of every installed package), typecheck, lint, unit tests, a production build, the bundle budget (`scripts/bundle_budget.mjs`, per-file size limits for `dist/assets/`) and the end-to-end suite; Playwright traces are kept as an artifact when a test fails. Dependencies are pinned exactly, installed from the lockfile with install scripts disabled (`.npmrc`), and the actions are pinned to commit hashes with read-only credentials that are not persisted. Dependabot opens weekly update pull requests for npm packages and the actions. Node is pinned by `.nvmrc`.
...
- **Content Security Policy.** `index.html` carries a `<meta http-equiv="Content-Security-Policy">` (GitHub Pages cannot send headers): scripts, workers, styles and fonts only from the site itself, no inline scripts or styles (element styles go through the CSSOM), no plugins or embedded objects. `connect-src` stays open because `#url=` may point at any CORS-enabled host; only http(s) targets are accepted. The dev server allows inline styles because Vite injects CSS as `<style>` elements; builds and `vite preview` use the strict policy, and the end-to-end suite fails on any violation. `npm run e2e:webkit` (after `npx playwright install webkit`) runs the policy tests in WebKit, Safari's engine, which applies policies to workers and WebAssembly differently from Chromium; the worker keeps its own same-origin policy there too, so the page needs no `wasm-unsafe-eval` allowance. Nothing is loaded from third parties: the typeface, the WASM engine and the compression plugins ship with the site, and the page sends no `Referer` header. The threat model and the reporting process are in [SECURITY.md](../SECURITY.md).
- **GitHub Pages.** Turn on **Enforce HTTPS** in the Pages settings. Every push to `main` deploys, so protect the branch: require the CI check to pass before merging and limit who can push. Pages sends no `X-Frame-Options`, so the site can be framed; the viewer keeps no session state, so a framed copy can only show what the framing page could load itself.
- **Self-hosting.** Any static file server works. Serve `assets/` with `Cache-Control: public, max-age=31536000, immutable` (file names carry content hashes) and `index.html` plus `data/` with a short max-age. `.h5ad` files need `Accept-Ranges: bytes`; add CORS (`GET, HEAD`, exposing `Content-Range`, `Content-Length` and `Accept-Ranges`) when they live on another origin. Where headers are available, send them:
